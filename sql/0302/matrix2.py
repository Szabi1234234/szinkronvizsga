...
print("")
# Koncentrikus 2x2 körökben a legnagyobb összegű 4-es megkeresése

def telt_e(i, j):
    return (min(i, j, 19-i, 19-j) // 2) % 2 == 0

def legjobb_negy(matrix, feltetel):
    legjobb, koordinatak = 0, []
    # Vízszintes
    for i in range(20):
        for j in range(17):
            if all(feltetel(i, j+k) for k in range(4)):
                osszeg = sum(int(matrix[i][j+k]) for k in range(4))
                if osszeg > legjobb:
                    legjobb, koordinatak = osszeg, [(i, j+k) for k in range(4)]
    # Függőleges
    for i in range(17):
        for j in range(20):
            if all(feltetel(i+k, j) for k in range(4)):
                osszeg = sum(int(matrix[i+k][j]) for k in range(4))
                if osszeg > legjobb:
                    legjobb, koordinatak = osszeg, [(i+k, j) for k in range(4)]
    # Átló balról jobbra
    for i in range(17):
        for j in range(17):
            if all(feltetel(i+k, j+k) for k in range(4)):
                osszeg = sum(int(matrix[i+k][j+k]) for k in range(4))
                if osszeg > legjobb:
                    legjobb, koordinatak = osszeg, [(i+k, j+k) for k in range(4)]
    # Átló jobbról balra
    for i in range(17):
        for j in range(3, 20):
            if all(feltetel(i+k, j-k) for k in range(4)):
...
koordinatak = legjobb_negy(matrix, telt_e)

for i in range(20):
    for j in range(20):
        if not telt_e(i, j):
            print("  ", end=" ")
        elif (i, j) in koordinatak:
            print("\033[31m" + str(matrix[i][j]) + "\033[0m", end=" ")
        else:
            print(str(matrix[i][j]), end=" ")
    print()
